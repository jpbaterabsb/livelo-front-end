...
import Vue from 'vue'
import { getUserProfile, getUserRepos, getUserStars } from "../api"
        ...
        onSearch({ state }) {
            getUserRepos(state.search, 1)
                ...
                .catch(err => {
                    ...
                    this.commit("setErrorStatus", err.response.status)
                    if (err.response.status == 404)
                        this.commit("setErrorMessage", "Repositório não encontrado.")
        ...
        onSearchStars({ state }) {
            getUserStars(state.search, 1)
                ...
                .catch(err => {
                    ...
                    this.commit("setErrorStatus", err.response.status)
                    if (err.response.status == 404)
                        this.commit("setErrorMessage", "Estrelas não encontrado.")
